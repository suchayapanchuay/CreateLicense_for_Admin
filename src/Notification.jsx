...
/* THEME — Light (match CreateApiKey.jsx) */
const THEME = {
  pageBg: "#F5F8FF",
  stageBg: "#FFFFFF",
  card: "#FFFFFF",
...
const styles = {
  ...
  statusPill: (ok, connecting) => ({
    ...
    borderRadius: 999,
    border: `1px solid ${
      connecting ? "rgba(59,130,246,0.35)" : ok ? "rgba(34,197,94,0.35)" : "rgba(248,113,113,0.35)"
    }`,
    ...
    color: connecting ? "#2563EB" : ok ? "rgb(22,163,74)" : "#DC2626",
  }),
  ...
  filterCaret: { position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: THEME.textFaint },
  ...
  label: { color: THEME.textMut, fontSize: 13, fontWeight: 800, marginBottom: 4 },
  pill: {
...
export default function Notifications() {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      .then(([trials, orders]) => {
        const t =
          trials.status === "fulfilled" && Array.isArray(trials.value) ? trials.value.map(mapTrial) : [];
        const o =
          orders.status === "fulfilled" && Array.isArray(orders.value) ? orders.value.map(mapOrder) : [];
        setItems([...t, ...o]);
      })
    ...
    try {
      ...
      es.addEventListener("order_status_changed", (e) => {
        try {
          const d = JSON.parse(e.data);
          setItems((prev) =>
            prev.map((x) => (x.kind === "order" && x.id === d.id ? { ...x, status: d.status } : x))
          );
        } catch {}
      });
      es.onerror = () => {
  ...
  async function handleDelete(card) {
    ...
    const url =
      card.kind === "order"
        ? `${API_BASE}/orders/${card.id}`
        : `${API_BASE}/trial-requests/${card.id}`;
    try {
  ...
  const filtered = useMemo(() => {
    const list = items.filter(
      (n) =>
        filter === "all" ||
        (filter === "trial" && n.kind === "trial") ||
        (filter === "purchase" && n.kind === "order")
    );
    if (!q.trim()) return list;
    const kw = q.toLowerCase();
    return list.filter((n) =>
      [n.firstName, n.lastName, n.email, n.company, n.industry, n.country, n.message, n.status]
        .join(" ")
        .toLowerCase()
        .includes(kw)
    );
    ...
    <div key={`order-${n.id}`} style={styles.notiCard}>
      ...
      <div style={styles.grid2}>
        <div>
          ...
          <div style={styles.pill}>{`${n.firstName || ""} ${n.lastName || ""}`.trim() || "-"}</div>
        </div>
        <div>
      ...
      <div style={styles.grid2}>
        <div>
          ...
          <div style={styles.pill}>{n.status || "pending"}</div>
        </div>
        <div>
          <div style={styles.label}>Items</div>
          <div style={styles.pill}>{Array.isArray(n.items) ? n.items.length : 0}</div>
        </div>
      </div>
      ...
      <div style={styles.btnRow}>
        ...
        <button style={styles.btnGhost} onClick={() => handleDelete(n)}>
          Delete
        </button>
      </div>
    </div>
        ...
        <div style={styles.stage}>
          ...
          <div style={styles.titleRow}>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              <span style={styles.statusPill(connected, connecting)}>
                {connecting ? "CONNECTING…" : connected ? "LIVE" : "OFFLINE"}
              </span>
              {errorMsg ? (
                <span style={{ marginLeft: 8, color: THEME.textFaint, fontSize: 12 }}>{errorMsg}</span>
              ) : null}
            </div>
          </div>
