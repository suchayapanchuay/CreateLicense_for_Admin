...
/* ===== Dark frame (page) + Light notification panel ===== */
const COLORS = {
  // โทนเข้มของเฟรม/เพจ
...
const styles = {
  ...
  bellBadge: {
    ...
    background: "#EF4444",
    color: "#fff",
    // ให้ตัดกับพื้นหลังเข้มเดิม
    border: `2px solid ${COLORS.stageBg}`,
  },
  ...
  notiPanelWrap: {
    ...
    borderRadius: 10,
    boxShadow: "0 14px 34px rgba(0,0,0,.15)",
    color: LIGHT.text,
    zIndex: 60,
    overflow: "hidden",
  },
  ...
  notiClose: { cursor: "pointer", color: LIGHT.textMut },
  ...
  notiList: { maxHeight: 560, overflowY: "auto", background: LIGHT.bg },
  notiItem: {
    padding: "16px",
    borderBottom: `1px solid ${LIGHT.border}`,
    background: LIGHT.bg,
  },
  notiBadge: { fontSize: 13, color: LIGHT.textMut, marginBottom: 6 },
  notiClient: { fontSize: 18, fontWeight: 800, marginBottom: 2, color: LIGHT.text },
  notiMetaRow: {
    display: "flex",
...
/**
 * Topbar ใช้ได้ทุกหน้า (ไม่มีช่อง Search ตามไฟล์ที่ส่งมา):
 * - ไอคอน Notifications + badge
 * - แผงรายการ (สว่าง) พร้อม SSE
 */
export default function Topbar({
  onSearchChange,      // (text) => void (ไม่ได้ใช้ในไฟล์นี้)
  ...
  const unreadCount = useMemo(
    () => items.filter((n) => !seenKeys.has(keyOf(n))).length,
    [items, seenKeys]
  );
  ...
  useEffect(() => {
    ...
    const onEsc = (e) => {
      if (e.key === "Escape") setShowNoti(false);
    };
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      .then(([trials, orders]) => {
        const t =
          trials.status === "fulfilled" && Array.isArray(trials.value)
            ? trials.value.map(mapTrial)
            : [];
        const o =
          orders.status === "fulfilled" && Array.isArray(orders.value)
            ? orders.value.map(mapOrder)
            : [];
        setItems(
          [...t, ...o].sort(
    ...
    try {
      ...
      es.addEventListener("trial_request", (e) => {
        try {
          ...
          setItems((prev) => [mapTrial(d), ...prev].slice(0, 300));
        } catch {}
      });
      es.addEventListener("order_created", (e) => {
        try {
          const d = JSON.parse(e.data);
          setItems((prev) => [mapOrder(d), ...prev].slice(0, 300));
        } catch {}
      });
      es.addEventListener("order_status_changed", (e) => {
        try {
          ...
          setItems((prev) =>
            prev.map((x) =>
              x.kind === "order" && x.id === d.id ? { ...x, status: d.status } : x
            )
          );
        } catch {}
      });
  ...
  return (
    <div style={styles.wrap}>
      ...
      <div style={{ position: "relative" }}>
        ...
        >
          {/* ไอคอนแจ้งเตือนเป็น "สีดำ" ตามที่ขอ */}
          <IoMdNotificationsOutline size={24} color="#000000" />
        ...
        {showNoti && (
          <div style={styles.notiPanelWrap} ref={notiRef}>
            ...
            <div style={styles.notiList}>
              {filtered.length === 0 ? (
                <div style={{ padding: 16, color: LIGHT.textFaint, fontStyle: "italic" }}>
                    ...
                    <div key={keyOf(n)} style={styles.notiItem}>
                      ...
                      <div style={styles.notiClient}>
                        {`${n.firstName || ""} ${n.lastName || ""}`.trim() ||
                          n.company ||
                          n.email ||
                          (n.kind === "order" ? "Order" : "Trial")}
                      </div>
                      <div style={styles.notiMetaRow}>
                        {n.email ? <span><strong style={{ color: LIGHT.textMut }}>Email:</strong> {n.email}</span> : null}
                        {n.company ? <span><strong style={{ color: LIGHT.textMut }}>Company:</strong> {n.company}</span> : null}
                        {n.kind === "order" && n.status ? <span><strong style={{ color: LIGHT.textMut }}>Status:</strong> {n.status}</span> : null}
                      </div>
                      <div style={{ ...styles.notiMetaRow, marginTop: 4 }}>
                        {n.created_at ? <span><strong style={{ color: LIGHT.textMut }}>Created:</strong> {new Date(n.created_at).toLocaleString()}</span> : null}
                      </div>
